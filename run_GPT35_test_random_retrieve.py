...
from utils.custom_parse import our_pred_parse
from prompting import get_prompt, conversion, table_prompt
from gpt35_prompting_temp import get_our_prompt, our_slots_prompt
    ...
    for data_item in tqdm(selected_set):
        ...
        if use_gold: # 이전 dialogue context 가 gold context 일 때 (분석 용도)
            prompt_text = get_prompt(
                data_item, examples=retriever.random_examples(data_item, k=NUM_EXAMPLE))
        else:
            ...
            examples = retriever.random_examples(
        ...
        print(prompt_text.replace(conversion(our_slots_prompt), ""))
        ...
        while not complete_flag:
            try:
                ...
                try:
                    ...
                    # 5번을 돌려 parse_error check 해보고 계속 그러면 종료
