...
from davinci003_completion import gpt35_completion
from ada_completion import ada_completion
from babbage_completion import babbage_completion
from gpt35_completion import gpt35_turbo_completion
from utils.sql import sql_pred_parse, sv_dict_to_string
from utils.custom_parse import our_pred_parse, our_pred_cot_parse
from prompting import get_prompt, conversion, table_prompt
from gpt35_prompting_temp import get_our_prompt, our_slots_prompt, traditional_prompt, get_each_cot_prompt, get_cot_augmented_prompt
...
# 각 설정마다의 selection pool이 정해져 있음. 이 중에서 retriever 가 검색해서 examplars 로 사용
...
def run(test_set, turn=-1, use_gold=False):
    ...
    for data_idx, data_item in tqdm(enumerate(selected_set)):
        n_total += 1
        ...
        if use_gold: # 이전 dialogue context 가 gold context 일 때 (분석 용도)
            ...
            # gpt35 completion for each example to generate CoT prompt            
            for example_id, example in enumerate(examples):
                cot_prompt = get_each_cot_prompt(example)

                example_flag = False
                example_error = 0
                while not example_flag:
                    try:
                        cot_completion = gpt35_turbo_completion(cot_prompt)
                        example["cot_completion"] = conversion(cot_completion, reverse=True)
                    except Exception as e:
                        example["cot_completion"] = ""

                        if e.user_message.startswith("This model's maximum context length"):
                            print("per example CoT prompt overlength")
                            example_flag = True
                        else:
                            print("per example CoT generation error")
                            example_error += 1
                            if example_error > 3:
                                example_flag = True        
                ...
                print(f"------------------{example_id}-------------------")
                print(example["cot_completion"])

                
            # add test instance with cot_completion
            prompt_text = get_cot_augmented_prompt(data_item, examples, given_context=predicted_context)

        print(prompt_text.replace(conversion(traditional_prompt), ""))
        ...
        while not complete_flag:
            try:
                ...
                if e.user_message.startswith("This model's maximum context length"):
                    ...
                    examples = examples[1:]
                    prompt_text = get_cot_augmented_prompt(
                        data_item, examples, given_context=predicted_context)
                else:
                    # throughput too high
                    timer.sleep(10)
            else:
                try:
                    temp_parse = our_pred_cot_parse(completion)
                except:
                    parse_error_count += 1
        ...
        try:
            predicted_slot_values = our_pred_cot_parse(completion) # a dictionary
        except:
